...
export class SupabaseProfileRepository implements IProfileRepository {
    ...
    // Returns the row id (DB-generated for new items) so callers can attach
    // follow-up writes — e.g. the background normalization result.
    async saveExperience(userId: string, experience: WorkExperience): Promise<string> {
    ...
    // Persist the background normalization result ("polished profile") for an
    // experience row. `sourceHash` is the hash of the description text the
    // normalization was computed from — readers compare it against the
    // current description to detect staleness.
    async saveExperienceNormalized(
        id: string,
