...
- `assertOutreachSpecificity(output, data, mode)` — outreach email (`mode='both'`) must mention the target company (full stored name OR its ≥4-char first token, so "NordPay" satisfies "NordPay Fintech Ltd") AND ≥1 candidate proper noun (own company / role / project / certification / award / school, **plus product/module nouns harvested from normalized bullets** — e.g. "Ditio Core", "Batch Camera Module"). LinkedIn note (`mode='either'`) needs at least one because of the 280-char limit.
