...
- **`/api/normalize-item`** — "polished profile" normalization (migration 015). Fired in the background when a profile experience is saved with a changed description (hash-guarded in `ExperienceSection`/`ProfileSetupScreen`): raw brain dump (English/Bangla/Banglish) → `{ bullets, skills, gaps }` stored in `experiences.normalized` beside the raw text (never replacing it). Flash-Lite, temp 0, strict `json_schema`, 20s deadline (`OpenRouterProfileNormalizer`). Telemetry kind `'normalize'` is EXCLUDED from the overall daily cap (profile edits must not starve paid generations) and has its own 40/day per-kind cap. The optimizer's user prompt sends `canonicalBullets` as primary evidence when present (`resumeOptimizerPrompts.buildUserPrompt`) — generations start from a stable pre-cleaned form instead of re-interpreting Banglish each time. OpenRouter-only (no legacy Gemini sibling): the endpoint 503s on the legacy path and clients treat polish as unavailable. Currently wired for **experiences only**; projects/extracurriculars are the designed follow-up (same recipe).
