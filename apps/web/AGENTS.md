...
- **The pill has TWO exits and they are not the same thing.** *Hide* folds it away for the session (session state, non-destructive — the modal still resumes). *Cancel this transaction* calls `voidTxn`, which frees a slot against the 5-per-24h cap AND releases the TrxID for re-entry, and is offered ONLY for `underpaid | msisdn_mismatch_review | expired` — never plain `pending`, where the watcher may be seconds from matching a real payment. The old single "Dismiss" did neither: it deleted the localStorage crumb while leaving the row pending, so the customer lost the payment from view *and* could not resubmit the same correct TrxID afterwards. localStorage is now only a first-paint hint.
