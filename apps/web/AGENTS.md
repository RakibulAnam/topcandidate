...
**Name:** TOP CANDIDATE (two-word wordmark: ink + saffron), preceded in-product by the **logo mark** — an ink "C" arc with a saffron "T" crossbar (`public/logo-mark.svg`, transparent background, rendered by `components/ui/LogoMark.tsx` with `alt=""` because the wordmark text beside it already names the brand). Sized by height to sit with the wordmark: `h-6` next to `text-lg`, `h-8` next to `text-2xl`. No "R" badge, no filled square tile — with ONE scoped exception, the app icon (below).
