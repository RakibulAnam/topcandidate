...
6. `reorderLeadBulletByJDFit` — within each item, promote the most JD-aligned bullet to position 0 (the recruiter's highest-attention spot).
7. `reorderProjectsByJDFit` — reorder *whole projects* by aggregate JD overlap. Stable; experience stays chronological because recruiters expect a timeline. `ResumeService.mergeOptimizedData` consumes the optimizer's output order for projects via `reorderProjectsByOptimizer`.
...
**Brand-name fidelity (`prompts/brandFidelity.ts`).** Local brand names are repaired against the candidate's OWN spelling: measured 2026-08-04, the normalizer turned "bkash" into "bakesh" roughly 1 run in 10, which zeroes ATS exact-match on the best keyword a BD fintech candidate has and reads as market illiteracy to a Dhaka recruiter. Two passes — casing ("bkash" → "bKash") and near-miss repair within 2 edits ("bakesh" → "bKash") — applied in the normalizer (where the corruption originates, and whose bullets are STORED so every later artifact inherits them) and again over optimizer prose as a net. **Safety rule: it can only ever CORRECT a brand already present in the candidate's evidence.** A brand absent from their words is never introduced, re-cased, or touched — otherwise "spelling repair" becomes a fabrication vector. Regression: `ai:selftest`.
