...
JSON handling: the **toolkit + interview generators use `response_format: json_schema`** (OpenRouter structured outputs — the provider enforces the shape, preventing the truncation/malformation that large *bilingual* interview JSON hit under `json_object`; this is the equivalent of the legacy Gemini `responseSchema`). The **optimizer + outreach use `json_object`** with the shape embedded in the prompt. All wrap the call in the deadline-bounded `withRetry` (2 attempts, never retries a timeout — `OpenRouterClient.ts`). The single-artifact interview gets a 55s deadline (it runs alone on `/api/toolkit-item`, so it can afford a retry of its slow ~25-30s bilingual generation); the toolkit gets 48s (parallel with the optimizer's 30s on `/api/optimize`). `max_tokens` is 8000 for the toolkit + interview (bilingual output is token-heavy).
