...
| **Customer purchase status pill** — navbar widget that tracks a submitted purchase via Supabase Realtime (sub-second) + a 20s fallback poll, no time cap; shows a 3-step timeline (Submitted → Verifying → Credits added / Needs attention) and fires `onCredited` so the credits badge refreshes without a reload | `VerifyingPurchasePill.tsx`, `purchaseStatusClient.ts` (`subscribeToPurchase`), `openPurchaseStore.ts` | shipped |
...
**Outreach email + LinkedIn note rules are written once (2026-09-27).** `outreachRules(mode)` and `linkedInRules(mode)` in `prompts/toolkitPrompts.ts` are spliced verbatim into BOTH the combined system instruction (as ARTIFACT 2 / 3) and the per-item `buildOutreachSystemInstruction(mode)` / `buildLinkedInSystemInstruction(mode)`, so a regenerate cannot drift from the first pass — edit the rule builder, never one path. The contract they encode: the email is **60–110 words (70–120 in stretch mode) in three blank-line-separated paragraphs**, naming the target company in the first paragraph (hook = the single strongest proof with its number and where it happened, one clause that they're applying; bridge = why it maps to what the JD is actually worried about, at most one more specific; ask = one fresh sentence under 20 words), subject ≤ 60 chars and findable by role title; the note is **120–200 chars, hard cap `LINKEDIN_MAX` = 280**, one concrete thing done plus one reason to connect, and must not open "As a <title> at <company>". Why the rewrite: sampled across three personas and both paths, every email was a single-block CV dump of 3–5 achievements, four of six notes opened on the banned title pattern, and 6 of 6 asks were the old prompt's own example sentence copied verbatim — so the new rules carry examples that are explicitly about a *different* candidate and say the ask shapes are shapes, not text. **Names as people say them:** `cleanAnchor()` in `toolkitContext.ts` strips trailing legal suffixes (Ltd / Limited / PLC / Inc / Pvt / AS…) and parenthetical qualifiers, `buildCandidateAnchors` returns the cleaned form alongside the stored one, and the target-company match in `assertOutreachSpecificity` accepts the suffix-stripped name too — so "at Shohoz" satisfies a stored "Shohoz Ltd" and "during my internship at Bengal Analytics" satisfies "Bengal Analytics (internship)". The `NAMES YOU MUST USE` list the model copies from shows only the cleaned forms. The `Company:` header in the toolkit, outreach and LinkedIn user prompts shows the cleaned target name too, so the model never sees the suffix it is told not to write. Cleaning never returns a stub under four characters ("ABC Ltd" stays whole), so the guard is never loosened to a substring that could match by accident. Durations count as metrics — the rules say to state "N years" only when the evidence dates give it, because a "Two years…" example hook got copied as a tenure onto candidates with six. **`stripEmailChrome(body, fullName)`** (also in `toolkitPrompts.ts`) runs on every outreach body before the guards, on the combined path, its repair, and the per-item generator: it removes a leading greeting line and a trailing signoff block (bare closing word and/or the candidate's own name, within the last three short lines) because the contract is body-only and a model-written "Best,\nName" ships as a duplicated signature. `scripts/quality-audit.ts artifacts` checks the new shape (paragraph count, word band, no stock ask, no title opener).
...
**An in-flight payment is a SERVER resource, not modal state.** `purchaseRepository.getOpenPurchase()` — newest row whose status is `pending | underpaid | msisdn_mismatch_review`, created inside 24h, with a `payment_reference` — is the single answer to "does this user have a payment in flight". It is cached in `infrastructure/api/openPurchaseStore.ts` (a module-level store, NOT React context, because the pill mounts in two trees: inside `DashboardShell` and inside the plain `Navbar` the builder uses) and read via `presentation/hooks/useOpenPurchase`. `AuthContext.signOut` calls `resetOpenPurchase()` — the store outlives the React tree, so a shared browser would otherwise hand the next account the previous one's payment; that is also why the store lives in infrastructure, since infrastructure must not import presentation.

Three rules fall out of that, and each one was a real bug:

- **`PurchaseModal` resumes, it does not restart.** On open with an open purchase it mounts straight into the takeover on that TrxID. `handleDismiss` must NOT call `reset()` — closing is a view action; the payment survives, so the panel must too. State is cleared only where it is genuinely finished (`finishAndClose`, and a corrected resubmit). Before this, closing mid-verification wiped phase/verdict/attempts and reopening gave a blank form.
- **The verify window is measured from when the PURCHASE was created (`trackedSince`), not when the sheet opened.** `VERIFY_WINDOW_MS` is a grace period for the watcher to see the bKash SMS; a purchase resumed twenty minutes later has already had it, so `wait = max(0, WINDOW - age)`. A flat window made every reopen sit through 20s of spinner before the screen would say anything.
- **A verdict is not a row status, and the pill needs it anyway.** `pending` covers both "submitted five seconds ago" and "we looked eighty minutes ago and no such payment exists", so a pill keyed on status alone spins on both — beside a modal that has already said the payment never arrived. `openPurchaseStore` therefore caches the last DIAGNOSIS alongside the open purchase, on its **own subscription channel** (`subscribeOpenPurchaseVerdict`): re-emitting on the purchase channel does nothing, because the value is the same object reference and React bails out of the re-render. The modal publishes every verdict it derives; the pill fetches its own once, only if nobody has one and the purchase is past `DIAGNOSE_AFTER_MS`. On `nothing_found` / `watcher_stale` the chip, the timeline step, and the footer copy all stop claiming work is in progress — a spinner over a diagnosed dead end is the same lie the builder's progress panel used to tell.
- **The pill has TWO exits and they are not the same thing.** *Hide* folds it away for the session (session state, non-destructive — the modal still resumes). *Cancel this transaction* calls `voidTxn`, which frees a slot against the 5-per-24h cap AND releases the TrxID for re-entry, and is offered ONLY for `underpaid | msisdn_mismatch_review | expired` — never plain `pending`, where the watcher may be seconds from matching a real payment. The old single "Dismiss" did neither: it deleted the localStorage crumb while leaving the row pending, so the customer lost the payment from view *and* could not resubmit the same correct TrxID afterwards. **That crumb is gone entirely** — `PENDING_PURCHASE_KEY`, its DOM event and its read/write/clear helpers were deleted, because a browser-local guess that can outlive, contradict or be deleted independently of the payment is precisely the bug. `AuthContext.signOut` still `removeItem`s the legacy key for existing installs.
...
**Name:** TOP CANDIDATE (two-word wordmark: ink + orange), preceded in-product by the **logo mark** — an ink "C" arc with an orange "T" crossbar (`public/logo-mark.svg`, transparent background, rendered by `components/ui/LogoMark.tsx` with `alt=""` because the wordmark text beside it already names the brand). Sized by height to sit with the wordmark: `h-6` next to `text-lg`, `h-8` next to `text-2xl`. No "R" badge, no filled square tile — with ONE scoped exception, the app icon (below).

**Palette** (defined in `src/index.css` under the Tailwind v4 `@theme` directive — `--color-<group>-<shade>` tokens; no `tailwind.config`):
- `brand-*` — Ink (deep navy, 700 = `#0F1B2D`, the logo mark's ink). Primary text, buttons, dark panels. Dark enough to read as ink — this is not a "blue palette"; don't add lighter blues around it.
- `accent-*` — Orange (500 = `#E8743B`, the logo mark's accent; 400 = `#EC8752`). Single accent — CTAs, highlights, active-state hints, "CANDIDATE" in the wordmark. Use sparingly (≤ 10% of pixels). Palette moved from near-black `#1A1812` + orange `#C57B17`/`#E59321` to the logo colours on 2026-09-25.
...
**Scoped exception — bKash magenta (`#E2136E`):** `PurchaseModal.tsx` is a bKash **payment surface**, so bKash's brand magenta is the action color for that ONE component — the trust chip, the numbered step markers, the copy-number button, and the primary "Submit" CTA all use `#E2136E` (deep `#B80E5D` on hover); emerald `#10B981` marks valid/success. Orange is intentionally NOT used inside this modal so the user feels they're in a bKash-branded flow. Layout: a two-column **split-sheet on desktop** (cream receipt panel + white action panel) and a **keyboard-aware bottom sheet on mobile** (receipt collapses to a one-line disclosure ribbon; the sheet is sized to `window.visualViewport` so the TrxID input + docked CTA stay above the soft keyboard). Do NOT extend bKash magenta to any other screen, button, or component.

**Scoped exception — dashboard hero gradients + toolkit tints (2026 redesign):** the dashboard area (`DashboardScreen` Home, `components/dashboard/*`) uses a deliberate, narrow set of gradients: the dark "Start a new application" CTA card's animated amber glint (`@keyframes glintMove` in `index.css`) and its amber icon square (`linear-gradient(135deg,#EC8752,#C95D27)`), plus the cream Master Resume banner (`linear-gradient(120deg,#FFFDF8,#FBF4E4)`). The same exception covers the **5 muted per-artifact toolkit-chip tints** on that dark card — Tailored Resume (amber), Cover Letter (coral), Recruiter Email (green), LinkedIn Message (blue `#9DB8DF`), Interview Prep (purple `#B7A3D8`) — the only place blue/purple appear, always as low-opacity semantic labels on the dark surface. The same **tint formula** (14% fill / 32% border / full-strength glyph on ink) is reused ONCE more, on the **job-discovery board** (`components/dashboard/JobDiscovery.tsx`) — six muted per-angle tints on a second dark `brand-700` panel, which is what makes the two ink surfaces read as one family. That board deliberately does NOT reuse `glintMove`: the sweep is what makes the primary CTA feel alive, and spending it twice on one page would cost the CTA its distinction. **These two dark panels are now the whole exception.** Everywhere else the flat Orange/Ink/Stone rules hold; do NOT extend gradients, blue/purple, or the tint formula to any further surface.

**Scoped exception — the app icon is the logo mark on a square tile (brand assets adopted 2026-09-25, replacing the 2026-08-11 "TC" monogram):** the favicon / apple-touch-icon is the ONE place a filled square tile is allowed, because the slot is a square by definition and the two-word wordmark is illegible at 16–32px. `public/favicon.svg` is the master: the mark scaled 1.2× on a cream (`#F4F1EA`) rounded tile (rx 44/200). Shipped as `favicon.svg` + `favicon.ico` (16/32/48) + `apple-touch-icon.png` (180px, **square and unrounded** — iOS masks it itself and a pre-rounded tile gets rounded twice), wired with three `<link>` tags in `index.html`. The mark's colours (ink `#0F1B2D`, accent `#E8743B`) are the same values as the `brand-700` / `accent-500` tokens; the SVGs hard-code them, so change both together.

**This exception is the ICON SLOT ONLY.** In-product the logo is the transparent mark + two-word wordmark; never a filled tile in the top bar, a nav element, an avatar, a loading state, or an empty state.
...
- **Form wizards** (`ProfileSetupScreen`, `BuilderScreen`) use a sticky left
  ...
  marker is orange, completed is ink, untouched is charcoal.
...
- **Form primitives** (defined in `components/FormSteps.tsx`, shared across
  ...
  - `TipCard` — always-on "Quick guide" panel (orange-tinted) above form
  ...
  - `WritingGuide` — friendlier alternative to `TipCard` used at the top of
    ...
    *reassurance hero* ("write it however feels natural — the AI polishes
    ...
    structured fields.
  - `MiniGuide` — single-paragraph orange callout for sparse credential
