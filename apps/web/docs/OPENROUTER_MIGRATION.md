...
>
> **Interview-prep fix (2026-06-10):** in prod, interview prep kept getting blocked by the fabrication guard (e.g. "Objective-C" flagged) — wrong for prep, which should probe JD tech the candidate must rehearse. Removed the fabrication + anchor-coverage hard-fails on interview questions (kept them on cover letter / outreach / LinkedIn — those are claims to an employer). Also moved the **toolkit + interview to `json_schema` structured outputs** (large bilingual JSON was truncating/malforming under `json_object`) and gave the single-artifact interview a 55s deadline + 8000 `max_tokens`. Realistic-JD hot path: 3/3 all-artifacts; single-artifact interview 5/5.
>
...
| **Toolkit** (bilingual EN/BN) | `/api/toolkit` *(split off `/api/optimize` 2026-06-11)* | cover letter + outreach + LinkedIn + interview Qs → JSON | ~15K in / 6K out |
...
*Strategy + pricing researched and verified 2026-06-09. Status: SHIPPED (cutover complete, live in production). Re-verify model slugs/prices before changing any chain.*
