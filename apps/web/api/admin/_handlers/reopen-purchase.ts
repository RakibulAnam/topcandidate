// POST /api/admin/reopen-purchase
// Body: { purchaseId, reason }
// Flips expired/failed back to pending. Resets created_at so the cron
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { error } = await supabase.rpc('admin_reopen_purchase', { p_purchase_id: purchaseId, p_reason: reason });
  if (error) {
    const msg = error.message ?? '';
    if (msg.includes('purchase_not_found')) { res.status(404).json({ error: 'Purchase not found' }); return; }
    if (msg.includes('not_reopenable')) { res.status(409).json({ error: 'Cannot reopen from this status.', code: 'not_reopenable' }); return; }
    console.error('[admin/reopen-purchase] failed:', msg);
    res.status(500).json({ error: 'Reopen failed.' });
    return;
  }

  await recordAuditAction(supabase, {
    action: 'reopen_purchase',
    targetKind: 'purchase',
    targetId: purchaseId,
