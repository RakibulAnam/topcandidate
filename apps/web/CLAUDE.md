...
3. **Brand constraints are non-negotiable.** No gradients. No generic blue / indigo / purple palettes. Accent color is the logo orange (`accent-*`, 500 = `#E8743B`); primary ink is `brand-*` (deep navy, 700 = `#0F1B2D`); neutrals are `charcoal-*` (warm stone). The wordmark is two words — "TOP" in `brand-700`, "CANDIDATE" in `accent-500` — preceded by the logo mark (`public/logo-mark.svg` via `components/ui/LogoMark.tsx`, transparent, never a filled tile). Never a square badge/tile **in the product UI**. **Three scoped exceptions only** (do not extend any of them elsewhere): bKash magenta `#E2136E` inside `PurchaseModal.tsx`; the **2026 dashboard redesign** hero/master gradients + 5 muted per-artifact toolkit-chip tints (incl. blue/purple) on the dark "Start a new application" card, **and the same tint formula on the second dark panel, the job-discovery board** (`components/dashboard/JobDiscovery.tsx`, 6 per-angle tints) — those two ink panels are the whole of it; and the **app icon** — `public/favicon.svg` + `favicon.ico` + `apple-touch-icon.png` put the logo mark on a square cream tile, because a favicon slot is a square and the wordmark cannot be read at 16px. That last one is the icon slot ONLY: in-app the logo is always the transparent mark + wordmark. All three documented in `AGENTS.md` §10. Fonts are `Source Serif 4` (display) + `Instrument Sans` (UI), swapped in with that redesign.
