...
| `GET /api/purchase-ops/verify-txn` | `purchase-ops/_handlers/verify-txn.ts` | **Why is my purchase still pending?** Wraps `diagnose_pending_purchase` (migration 028). Returns a `verdict` — `likely_typo` / `awaiting_sms` / `watcher_stale` / `nothing_found`, or the already-settled status — so `PurchaseModal` can say something specific instead of closing behind a spinner. `404` if the TrxID isn't the caller's. **Returns nothing at all about the unclaimed payment it matched against** — no TrxID, amount, or sender, masked or otherwise (migration 029; the verdict is the only output). No `vercel.json` rewrite — called at this path directly. |
| `POST /api/purchase-ops/void-txn` | `purchase-ops/_handlers/void-txn.ts` | Retire the caller's own `pending` row (`void_pending_purchase`) so an "Edit & resubmit" after a mistyped TrxID doesn't consume another of their 5 pending slots per 24h. `409` if it's no longer pending. No rewrite. |

> **AI failure telemetry (migration 020, 2026-08-04):** an AI generation that fails after the client-side model chain is exhausted answers `502` with `{ error }` (`code: 'refund_failed'` on `/api/optimize` only, when the credit refund also failed). The *diagnosis* is not in the response body — [`api/_lib/aiTelemetry.ts`](../../apps/web/api/_lib/aiTelemetry.ts) writes a normalized `error_code` (`rate_limit`, `quota_exhausted`, `timeout`, `auth`, `schema_invalid`, `guard_rejected`, `unknown`, …), a redacted `error_message`, and `model_attempts` (the chain as actually tried) to `ai_call_log`. Read it via `v_ai_failures_daily` / `v_ai_model_health`, not from the client.

## Webhook endpoints (HMAC — Flutter watcher)

| Endpoint | File | Purpose |
| --- | --- | --- |
| `POST /api/confirm-purchase` | `confirm-purchase.ts` | **The payment webhook.** Matches the TrxID, validates amount + sender, grants credits via `confirm_purchase`. Also serves **heartbeat mode** (`{ kind: 'heartbeat', deviceId, … }` → `record_watcher_heartbeat`, migration 028) on the same secret and signature, because the 12-function cap forbids a new file. See [`webhook-confirm-purchase.md`](webhook-confirm-purchase.md). |
...
Helpers: auth [`api/_lib/auth.ts`](../../apps/web/api/_lib/auth.ts); rate-limit / daily-cap [`api/_lib/rateLimit.ts`](../../apps/web/api/_lib/rateLimit.ts); AI provider factory [`api/_lib/aiFactory.ts`](../../apps/web/api/_lib/aiFactory.ts) (direct Google Gemini, gated on a single `GEMINI_API_KEY` — must be a **paid**-tier key, bounded by a Cloud Console spend-cap budget; every generator is `null` without it, which is what turns into the `503`s above); AI call telemetry [`api/_lib/aiTelemetry.ts`](../../apps/web/api/_lib/aiTelemetry.ts); admin auth [`api/admin/_lib/adminAuth.ts`](../../apps/web/api/admin/_lib/adminAuth.ts).
